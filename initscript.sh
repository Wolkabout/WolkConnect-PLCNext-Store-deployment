...
declare -A IMAGES
IMAGES[f177131a5397d6218c6c5400e45ca8651876a99c79c348a46afa475465cba5dc]=wolkabout-plcnext.tar.gz
IMAGES[23b4933452342936d334080fba91104b3184b60bc11de58632a65b581c79cf50]=wolkabout-plcnext-webui.tar.gz
IMAGES[55c4da5187920f9d611f6017a5505d075e3395b1a8c943c4d73f13057c4d20f0]=arm32v7-redis-alpine.tar.gz
...
start () 
{
  ...
  do
    ...
    then
      ...
      then
        ...
        $CONTAINER_ENGINE pull ${IMAGES[$IMAGE_NAME]} >> $APP_LOG 2>&1
        if [ ! $? -eq 0 ]
        then
          stop
          echo "$(date): Wasn't able to load $IMAGE_NAME from ${IMAGES[$IMAGE_NAME]}" >> $APP_LOG
  ...
  if $CONTAINER_ENGINE pod exists pod_${APP_UNIQUE_NAME}
    then
      ...
      if $CONTAINER_ENGINE ps -a --format {{.ID}} --filter label=io.podman.compose.project=${APP_UNIQUE_NAME} | grep -q .
      then
        echo "$(date): Started container:"
        $CONTAINER_ENGINE ps -a --filter label=io.podman.compose.project=${APP_UNIQUE_NAME} >> $APP_LOG
      else
        stop
        echo "$(date): Error while starting $APP_NAME" >> $APP_LOG
        exit 302
      fi
      ...
      then        
        ...
        stop
        echo "$(date): Error while starting $APP_NAME initially" >> $APP_LOG
        exit 301
      fi
  fi
